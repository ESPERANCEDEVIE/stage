 ...
 */
    ...
    public function getVisiteur($login,$mdp){
        $req = "select idVisiteur from visiteur where login='$login' and mdp='$mdp'";
        $rs = Pdoasso::$monPdo->query($req);
        $ligne = $rs->fetch();
        return $ligne;
    }

    public function newVisiteur($nom,$prenom,$numMobile,$majeur,$statut,$mail,$mdp,$verifmdp){
        $req = " INSERT INTO 'visiteur' VALUES ('nom','prenom','numMobile','majeur','statut','login','mdp') VALUES ('$nom','$prenom','$numMobile','$majeur','$statut','$mail','$mdp')";
        $rs = Pdoasso::$monPdo->query($req);
        $ligne = $rs->fetch();
        return $ligne;
    }
